...
export function getAllCampaigns({
  ...
  let query =
    "SELECT c.*, u.display_name as creator_name FROM campaigns c JOIN users u ON c.user_id = u.id WHERE c.status = ?";
  const params = ["active"];
...
export function updateCampaign(id, updates) {
  const { title, description, goalAmount, imageUrl, category, status } =
    updates;
  ...
  if (imageUrl !== undefined) {
    sets.push("image_url = ?");
    params.push(imageUrl ? imageUrl.trim() : null);
  }
  ...
  const query = `UPDATE campaigns SET ${sets.join(", ")} WHERE id = ?`;
  params.push(id);
